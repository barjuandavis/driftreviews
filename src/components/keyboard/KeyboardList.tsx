...
import KeyboardUICard from "./KeyboardUICard";

import { KeyboardPost, getAllKeyboard } from "../../api/prismic";
import { AnimatePresence } from "framer-motion";
import KeyboardNotFound from "./KeyboardNotFoundSection";
...
export default function KeyboardList() {
  const [mousepadData, setMousepadData] = useState<KeyboardPost[]>(
    [] as KeyboardPost[],
  );
  ...
  useEffect(() => {
    const fetchMouseData = async () => {
      setLoading(true);
      const mousepadData = await getAllKeyboard();
  ...
  const filteredMousepad = mousepadData
    .filter((mouse) => {
      const checks = [
        mouse.data.keyboard_name.toLowerCase(),
        ...
        <AnimatePresence>
          ...
          {filteredMousepad.length > 0 && !loading ? (
            filteredMousepad.map((mousepad) => (
              <KeyboardUICard key={mousepad.id} mousepadPost={mousepad} />
            ))
          ) : (
            <KeyboardNotFound />
          )}
